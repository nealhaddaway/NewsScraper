...
input <- read.csv("TheGuardian.csv")
...
for(i in 1:length(input$urls)){
  ...
  #print(paste0('Web page ', i,' successfully downloaded'))
...
for (i in 1:length(research_output$lead_img)) {
  if(output$lead_img[i]=='[No lead image]'){
    ...
    download.file(output$lead_img[i], destfile=paste0("lead_images/TheGuardian/page_", i,".jpg"), quiet = TRUE)

...
input <- read.csv("TheTelegraph.csv")
...
# save output
write.csv(output, 'TheTelegraphOutput.csv', row.names = FALSE)
...
for (i in 1:length(output$lead_img)) {
  if(output$lead_img[i]=='[No lead image]'){
    ...
    download.file(output$lead_img[i], destfile=paste0("lead_images/TheTelegraph/page_", i,".jpg"), quiet = TRUE)

...
dir <- list.files(path="./googleresults/DailyMirror", all.files=TRUE, 
                  full.names=TRUE)
farming <- list.files(path="./googleresults/DailyMirror/farming", all.files=TRUE, 
                      full.names=TRUE) %>%
  .[endsWith(., '.html')]
agriculture <- list.files(path="./googleresults/DailyMirror/agriculture", all.files=TRUE, 
                          full.names=TRUE) %>%
  .[endsWith(., '.html')]
crops <- list.files(path="./googleresults/DailyMirror/crops", all.files=TRUE, 
                    full.names=TRUE) %>%
  .[endsWith(., '.html')]
livestock <- list.files(path="./googleresults/DailyMirror/livestock", all.files=TRUE, 
                        full.names=TRUE) %>%
  .[endsWith(., '.html')]
foodsystem <- list.files(path="./googleresults/DailyMirror/foodsystem", all.files=TRUE, 
                         full.names=TRUE) %>%
  .[endsWith(., '.html')]
...
for (i in 1:length(source)){
  ...
  links <- html %>% 
    ...
    html_attr('href') 
  links <- unique(links[startsWith(links, 'https://www.mirror.co.uk')])
...
write.csv(output, 'googleresults/DailyMirror/DailyMirror_googleresults.csv', row.names = FALSE)

...
dir <- list.files(path="./googleresults/DailyMail", all.files=TRUE, 
                  full.names=TRUE)
farming <- list.files(path="./googleresults/DailyMail/farming", all.files=TRUE, 
                      full.names=TRUE) %>%
  .[endsWith(., '.html')]
agriculture <- list.files(path="./googleresults/DailyMail/agriculture", all.files=TRUE, 
                          full.names=TRUE) %>%
  .[endsWith(., '.html')]
crops <- list.files(path="./googleresults/DailyMail/crops", all.files=TRUE, 
                    full.names=TRUE) %>%
  .[endsWith(., '.html')]
livestock <- list.files(path="./googleresults/DailyMail/livestock", all.files=TRUE, 
                        full.names=TRUE) %>%
  .[endsWith(., '.html')]
foodsystem <- list.files(path="./googleresults/DailyMail/foodsystem", all.files=TRUE, 
                         full.names=TRUE) %>%
  .[endsWith(., '.html')]
...
for (i in 1:length(source)){
  ...
  links <- html %>% 
    ...
    html_attr('href') 
  links <- unique(links[startsWith(links, 'https://www.dailymail.co.uk')])
...
write.csv(output, 'googleresults/DailyMail/DailyMail_googleresults.csv', row.names = FALSE)
